...
Every training event hosted by NRIS has an associated collaborative document where participants can ask questions and make comments in real time during the course. A fully anonymized version of these documents is made available to the public after the event, available here: {ref}`collaborative-notes-archive`. 
