...
By combining the two pictures displayed in [Figs. 1](#Fig1) and [2](#Fig2), one can say that the execution of the parallelism, which is specified by the `parallel loop` construct, is mapped on the GPU-device in the following way: each streaming multiprocessor is associated to one gang of threads generated by the directive `gang`, in which a block of loops is assigned to. In addition, this block of loops is run in parallel on the CUDA cores via the directive `vector`. The description of these directives and others implemented in our OpenACC mini-application is summarized in the [Table 1.](#Table1).  
...
The description of the compute constructs and clauses used in our OpenMP mini-application is provided in the [Table. 1](Table1) together with those of OpenACC. For further OpenMP tutorials, we refer to a different scenario implemented in C, which can be found {ref}`here <ompoffload>`.
