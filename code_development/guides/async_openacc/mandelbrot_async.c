...
// Default number of blocks to split computation over
static const int DEFAULT_BLOCKS = 10;
...
int main (int argc, char** argv) {
  ...
  if (argc > 1) {
    if (strncmp (argv[1], "-h", 2) == 0 || strncmp (argv[1], "--help", 6) == 0) {
      printf("Usage: %s <width>x<height> <max iterations> <blocks> <output filename>\n", argv[0]);
  ...
  // Third argument is the number of blocks to split computation over
  if (argc > 3) {
    num_blocks = atoi (argv[3]);
  }
  // Fourth argument is the output filename to write PNG file to
  if (argc > 4) {
    if (strlen (argv[4]) > 127) {
      printf("\033[0;31mOutput filename to large!\033[0m");
      return EXIT_FAILURE;
    }
    strncpy (output_name, argv[4], strnlen (argv[4], 127) + 1);
  }
  ...
  #pragma acc data create(image[:width * height]) copyin(palette[:palette_size])
