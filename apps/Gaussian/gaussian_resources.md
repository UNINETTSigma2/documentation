...
As commented in the [performance-tips section](../../storage/performance-tips.md), there is an issue with very large temporary output files (termed RW files in Gaussian). It is advisable to slice them into smaller parts using the `lfs setstripe` command. 
